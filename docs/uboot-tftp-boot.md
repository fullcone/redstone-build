...
| `ERROR: Failed to allocate 0x... bytes below 0x1000000. device tree - allocation error` | After de-gzipping the kernel, raw image is ~28MB and loaded at 0x0, occupying 0–0x1c00000. U-Boot's default FDT relocation looks for free RAM below 0x1000000 (16MB) — none available. **Temporary fix (per session, do NOT `saveenv`)**: `setenv fdt_high 0xffffffff ; setenv initrd_high 0xffffffff ; bootm 0x02000000`. This lets U-Boot place FDT/initrd anywhere in RAM. **Don't persist these env vars** — historical bench notes warn they break ONIE and EdgeNOS boot paths. Long-term fix: change FIT image's kernel `Load Address`/`Entry Point` to a higher address (e.g. 0x4000000) so FDT space stays free. |
